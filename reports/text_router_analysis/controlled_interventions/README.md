...
| `GZ` | 固定为 `aT` | `pZ[aT]` | 只替换为零文本输出权重 |
| `GW` | 固定为 `aT` | `pW[aT]` | 只替换为其他类别文本输出权重 |
| `E0` / `E1` | 全部固定为专家 0 / 1 | 原生 `gT` | 比较专家整体质量 |
| `FZ` / `FW` | 固定为 `1-aT` | `pZ[aT]` / `pW[aT]` | 探索编号与输出权重的交互 |
...
图中 (a) 的差值为替换条件减目标条件，线为逐比较配对区间；(b) 为绝对 New17 AP×100，连接线仅连接同检查点的两个点估计；(c) 为主要被选专家的比例，采用局部放大刻度。

固定专家编号后，其他类别文本输出权重相对目标文本输出权重在三组检查点中都降低 New17 AP；零文本输出权重的方向为一负两正。这说明输出权重在这些固定检查点上能独立改变检测质量，但目标文本输出权重并未一致优于零文本输出权重。

在相同逐图 `gT` 下，专家 0 的 New17 AP 在三组检查点中都高于专家 1。原生路由在 seed 1 和 seed 2 却几乎全部选择专家 1；对应的反向选择 `FT` 分别提高 +2.3648 和 +0.6843 AP 点。这呈现了原生选择与共同权重条件下固定专家 New17 AP 排序的不一致，不能确定形成偏好的训练原因。专家编号只在各自检查点内有意义，也不表示专家 0 在每张图上都更好。
...
下表 AP 列均为诊断选择减固定专家 0，单位为 AP 点。

| seed | F1 不同的图像 | 改用专家 1 | 平均 F1 增量 | New17 AP 点差 | 65 类整体 AP 点差 |
| --- | ---: | ---: | ---: | ---: | ---: |
| 0 | 1277 | 555 | +0.01706 | +0.1938 | +0.1830 |
| 1 | 1227 | 570 | +0.01485 | −0.1632 | +0.0970 |
| 2 | 1145 | 584 | +0.01737 | +0.5181 | +0.3155 |

![局部专家机会与目标指标收益](figures/fig6_local_opportunity.svg)

New17 AP 点差两正一负，均值为 +0.1829 AP 点，未满足三组同向且均值至少 +0.5 AP 点的主指标目标。65 类整体 AP 点差则三组均为正，均值为 +0.1985 AP 点；本诊断没有计算这些点差的不确定性区间，不据此宣称显著收益。

逐图取最大 F1 本就保证该评分不下降，不能将它视为独立的学习成功。F1 相同的灰色区域只表示该评分无法区分两专家，不表示预测相同或均正确。逐图 65 类 F1 与 New17 AP 在类别范围、置信度处理、IoU 标准和图像聚合方式上均不同，当前结果不能将变化方向归因于其中一项。真实标注选择不是可部署路由器，也不构成所有自适应策略的 AP 上界。
