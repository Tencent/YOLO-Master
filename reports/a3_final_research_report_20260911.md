...
> 提交 `aac33de` 已把导出路由冻结为部署权威来源，并将 eager 漂移降为不隐藏的参考诊断；动态 INT8 扩展
> 继续冻结。
...
4. **本地实现已完成：**部署导出路由作为唯一选择源；误差预算只作诊断，不替换实际专家 ID。下一次云端
   复验权威路由与实际 dispatch 的逐调用一致性，同时保留 eager 参考漂移。
