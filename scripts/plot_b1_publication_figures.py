...
OUTPUT = Path(os.environ.get("B1_PUBLICATION_FIGURE_OUTPUT", DATA / "figures"))
FORMATS = tuple(
    ext.strip().lower() for ext in os.environ.get("B1_PUBLICATION_FIGURE_FORMATS", "svg").split(",") if ext.strip()
)
...
OPPORTUNITIES = {int(r["seed"]): r for r in rows("opportunity_summary.csv")}
...
def save(fig, stem, cn):
    ...
    for ext in FORMATS:
        fig.savefig(OUTPUT / f"{stem}.{ext}", dpi=190, bbox_inches="tight")
    plt.close(fig)
...
for cn in [False]:
    ...
    fig, axs = plt.subplots(
        1, 3, figsize=(10.5, 3.55), gridspec_kw={"width_ratios": [1.15, 1.15, 1]}, layout="constrained"
    )
    a, b, c = axs
    for ax in axs:
        setup(ax, cn)
    ...
    a.set_xlabel("Substituted - target New17 AP (points)")
    a.legend(
        [Patch(color=BLUE), Patch(color=ORANGE)],
        ["Zero text", "Other-class text"],
        loc="lower left",
        fontsize=8,
        frameon=False,
    )
    for seed in range(3):
        ...
        for expert, value in enumerate(x):
            b.annotate(
                f"{value:.3f}",
                (value, seed),
                xytext=(0, 10 if expert == 0 else -16),
                textcoords="offset points",
                ha="center",
                fontsize=8,
                color=BLUE if expert == 0 else ORANGE,
            )
    b.set_title("(b) Fixed-expert quality")
    b.set_xlabel("New17 AP (×100)")
    ...
    for seed in range(3):
        ...
        c.annotate(
            f"{dominant:.2f}%", (dominant, seed), xytext=(0, 10), textcoords="offset points", ha="center", fontsize=9
        )
    ...
    for seed in range(3):
        ...
        for key, col in [("images_E0_better", BLUE), ("images_E1_better", ORANGE), ("images_tie", GRAY)]:
            ...
            a.text(
                ...
                va="center",
                fontsize=8,
                color="#333" if col == GRAY else "white",
            )
            left += value
    ...
    a.set_title("(a) Per-image 65-class F1 comparison")
    a.set_xlim(0, 100)
    a.set_xticks([0, 25, 50, 75, 100])
    a.set_xlabel("Share of all images (%)")
    a.legend(
        [Patch(color=x) for x in [BLUE, ORANGE, GRAY]],
        ["E0 higher", "E1 higher", "Tie"],
        fontsize=8,
        frameon=False,
        ncol=3,
        loc="upper center",
...
for svg in OUTPUT.glob("fig[56]_*.svg"):
