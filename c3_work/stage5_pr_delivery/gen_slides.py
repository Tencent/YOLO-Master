...
blocks = [
    ...
    ("已知缺陷 cap<8", "0.conv/routing_network.2/dfl.conv 容量 < 最小候选 rank 4 → placement_plan.py:117 raise; 本次以 lora_exclude_modules 规避(子串匹配, 如实记录), 修复见 P2 issue 草稿", AMBER),
...
ev = [
    ...
    ["缺陷取证(P2)", "stage4_analysis/p2/planner_bug_issue.md", "cap<8 ValueError 源码行号 + 建议修复"],
]
tx, tw = 0.55, 7.7
colw = [2.1, 3.1, 2.5]
...
for name, statline, desc, c in sums:
    ...
    xx += 4.05
text(s, 0.7, 5.12, 12.0, 0.32, [P("待办 / 下一步", 12.5, RGBColor(0xAF, 0xC2, 0xD2))])
todo = [
    ("① 手动 push", "8 个本地 commit → fork lycyhrc/YOLO-Master:c3-vpeft-smoke(按 PR 四节)"),
    ("② P2 issue", "planner cap<8 修复 PR(草稿已备) + case-embedding 调研"),
    ("③ 可选扩展", "k=5/10/50/100 档位曲线(数据已就绪), GPU 低峰 run_supplement 即可"),
]
yy = 5.55
for t, d in todo:
    text(s, 0.7, yy, 12.0, 0.42, [[("▸ ", 12, CYAN, True), (t + "  ", 12, WHT, True), (d, 11, RGBColor(0xAF, 0xC2, 0xD2))]])
    yy += 0.5
text(s, 0.7, 6.92, 9.0, 0.25, [P("数字来源: stage3_matrix/runs(21 单元) · stage4_analysis(2026-09-07 定稿)", 10, RGBColor(0x8F, 0xA3, 0xB5))])
