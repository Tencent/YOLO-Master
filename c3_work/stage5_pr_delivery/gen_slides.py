...
blocks = [
    ...
    ("缺陷已修 cap<8", "0.conv/routing_network.2/dfl.conv 容量 < 最小候选 rank(4): 原先求解器放行 → plan 校验抛错 → V-PEFT 静默降级 legacy; 修复分支已推 fork 并另开上游 PR(C_cap 硬约束 + capacity_excluded 审计), 本报告数据仍是 exclude 规避口径未重跑 → 链接见末页①", GREEN),
...
ev = [
    ...
    ["缺陷修复(PR)", "分支 fix/vpeft-capacity-guard + 独立 PR", "C_cap 容量硬约束 + capacity_excluded 审计 + 未适配层冻结(2 commit · 链接见末页①)"],
]
tx, tw = 0.55, 7.7
colw = [2.1, 3.1, 2.5]
...
todo = [
    ("① 修复分支已 push", "fix/vpeft-capacity-guard(2 commit); 独立 PR 待发起"),
...
text(s, 8.15, 5.38, 4.3, 0.32, [[("① 上游修复 PR  ", 9.5, WHT, True), ("独立发起", 9.5, CYAN, True)]])
