...
- 已知缺陷 cap<8 层(0.conv/routing_network.2/dfl.conv)经 `lora_exclude_modules` 规避(strict=True 不降级)，P2 issue 素材。
...
3. **planner cap<8 缺陷**：param-cap<8 层直接 placement 抛 ValueError（见 P2 issue 草稿），用 exclude 规避。
...
- 全部本地 commit 已就绪；push 至 fork `lycyhrc/YOLO-Master:c3-vpeft-smoke` 由本人执行。
