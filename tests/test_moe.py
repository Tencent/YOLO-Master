...
def test_issue52_usage_gini_uniform_and_collapsed():
    """Issue #52: Gini is zero for uniform usage and high for expert collapse."""
    assert usage_gini([0.25, 0.25, 0.25, 0.25]) == pytest.approx(0.0)
    assert usage_gini([1.0, 0.0, 0.0, 0.0]) == pytest.approx(0.75)


def test_issue52_gini_balance_scheduler_clamps_and_updates_modules():
    """Issue #52: high Gini increases balance loss while clamp bounds are respected."""
...
def test_issue52_dynamic_schedule_is_disabled_by_default():
    """Issue #52: new dynamic schedule is opt-in for backward compatibility."""
    assert DEFAULT_CFG_DICT["moe_dynamic_schedule"] == "none"


def test_issue52_es_moe_get_gflops_reports_nonzero_total():
    """Issue #52: pruning script can collect ES_MOE FLOPs via get_gflops()."""
    m = ES_MOE(32, 32, num_experts=3, top_k=2)
    g = m.get_gflops((1, 32, 16, 16))
    assert isinstance(g, dict)
    assert g["total_gflops"] > 0
    assert apply_balance_loss_coeff(m, 1.5) >= 1
    assert m.balance_loss_coeff == pytest.approx(1.5)
