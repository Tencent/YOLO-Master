...
def test_capture_preserves_existing_hooks(model):
    head = model.model[-1]
    calls = []
    existing = head.register_forward_pre_hook(lambda *_: calls.append(True))
    try:
        with capture_head_inputs(head):
            model(torch.randn(1, 3, 64, 64))
        assert len(head._forward_pre_hooks) == 1
        assert calls == [True]
    finally:
        existing.remove()


def test_first_forward_rejects_partially_detached_features(tmp_path):
    class MixedFeatures(torch.nn.Module):
        def forward(self, x):
            return {"one2one": {"feats": [x, x.detach()]}}

    head = MixedFeatures()
    with pytest.raises(RuntimeError, match="inactive"), ExitStack() as cleanup:
        install_first_forward_check(head, 0.1, tmp_path / "first.json", cleanup)
        head(torch.randn(1, requires_grad=True))
    assert not head._forward_hooks
