...
四组均以 mAP50-95 选择结果行，选中行都在第 30 轮；表中的 mAP50 与 mAP50-95 来自同一行，
不把两个指标各自的独立最大值拼接在一起。MoT-P5 的 mAP50 单指标峰值为第 28 轮的
0.17315，表中保留第 30 轮配对值 0.17309。逐 epoch CSV 和曲线位于
[`training/`](training/)。这是同预算架构比较，不是充分收敛的 SOTA 复现。
