...
| P1：精度 / 显存 / 时长三维对照 | **两数据集三 seed 精度/显存/时长对照已完成** | 两数据集、两架构、三个 seed 的统一合同与队列已实现、验证并启动 | 按统一口径报告 AP、显存、GPU-hours、精度保留率及成本降低率，对照至少 50% 降本目标 |
...
| COCO / 50 / seed 1 | 冻结 DINOv3 + BN64 | 29.302 / 48.476 / 31.039 | 9.053 / 15.561 | 1.7668 | 10.6008 |
| COCO / 50 / seed 1 | Scratch | 26.976 / 40.071 / 28.582 | 40.845 / 43.539 | 5.4671 | 32.8027 |
| COCO / 50 / seed 2 | 冻结 DINOv3 + BN64 | 28.517 / 47.810 / 29.924 | 9.038 / 16.393 | 1.7585 | 10.5508 |
| COCO / 50 / seed 2 | Scratch | 27.012 / 40.036 / 28.849 | 40.829 / 43.539 | 5.4643 | 32.7860 |
...
| VisDrone / 120 / seed 2 | 冻结 DINOv3 + BN64 | 7.374 / 17.996 / 5.202 | 5.268 / 10.613 | 0.6028 | 3.6170 |
| VisDrone / 120 / seed 2 | Scratch | 11.068 / 21.603 / 10.028 | 13.673 / 18.293 | 1.3779 | 8.2673 |

**COCO 三 seed 汇总（2026-09-13）：** 六组均采用完整 train2017 118,287 / val2017 5,000、50 轮、batch 384、640 输入、六卡 A40、BF16 训练与 FP32 损失/评测，固定末轮按标准 bbox AP（maxDets=100）比较。BN64 / Scratch 的 AP 均值 ± 样本标准差为 **28.666 ± 0.576 / 27.087 ± 0.161**，训练 GPU-hours 为 **10.5952 ± 0.0419 / 32.8304 ± 0.0630**。按两组均值计算，精度保留率 **105.83%**，训练 GPU-hours 降低 **67.73%**，训练单卡峰值 allocated / reserved 分别降低 **77.86% / 63.46%**。

同 seed 配对的固定末轮 AP 差均值为 **+1.579 AP 点**，探索性 95% t 区间为 **[-0.191, 3.349]**，跨 0；逐 seed 训练 GPU-hours 降低率的区间为 **[67.53%, 67.92%]**（n=3、df=2）。周期最佳作为独立补充口径：BN64 三 seed 最佳轮次为 35/40/35，AP 均值 **28.800 ± 0.547**；Scratch 均为第 20 轮，AP 均值 **31.577 ± 0.033**。因此固定末轮与周期最佳呈现不同的精度排序，分别保留，训练成本比较仍使用完整 50 轮作业。上述为训练作业降本，Teacher 抽取与独立评测另列。

最后完成的 COCO Scratch seed 2 固定第 50 轮 AP 为 **27.012**，训练作业 **19,671.616 秒**，末轮 checkpoint SHA256 为 768680a93a900f5cf12ccc48f2a6e61a767c863cfc61ce916b3b285e13b1498d；执行提交 ee23d3edf04b3741e1a6441c57a9cc2ff4d2516c，配方 SHA256 c4baac70164f7b91e93d1d164864c35038c3fddfcea2d021a027bf176c3abee0。六 rank 各完成 15,450 次更新，50 轮损失有限、5,000 张标准评测与严格重载通过。完整记录与三 seed 统计见复现包 results/coco-SCRATCH-seed2.json、results/coco-three-seed-summary.json。

**主结果完成状态：** 两数据集三 seed 固定末轮主结果、周期最佳补充和成本审计均已完成。主表采用固定第 50/120 轮，周期最佳按同一快照频率另列。

**VisDrone 三 seed 汇总（2026-09-13）：** 六组均采用上述完整数据、120 轮、batch 96、640 输入和固定末轮官方 MATLAB DET 口径。BN64 / Scratch 的 AP 均值 ± 样本标准差分别为 **7.312 ± 0.192 / 11.026 ± 0.382**，训练 GPU-hours 为 **3.6239 ± 0.0120 / 8.3007 ± 0.0727**。全程单卡峰值 allocated 的三 seed 均值为 **5.276 / 13.702 GiB**，reserved 为 **10.688 / 18.076 GiB**。按两组均值计算，AP 保留率为 **66.31%**，训练 GPU-hours 降低 **56.34%**，allocated / reserved 分别降低 **61.50% / 40.87%**。

按 seed 配对的 AP 差均值为 **-3.714 AP 点**，探索性 95% t 区间为 **[-5.118, -2.311]**；逐 seed GPU-hours 降低率的均值为 **56.34%**，相应区间为 **[55.75%, 56.93%]**（n=3、df=2，使用样本标准差）。比值的均值与均值的比值分别记录：逐 seed AP 保留率均值为 66.40%，主汇总使用前述 66.31%。这组结果体现冻结方案以较低精度换取更低训练成本，训练作业口径达到 50% 降低目标；Teacher 一次抽取与独立评测仍按既定成本分类单列。机器可读汇总位于复现包 results/visdrone-three-seed-summary.json，含六份记录 SHA256、全部 AP/AR、显存和时长统计。

**新增 COCO 结果：** seed 1 两组均完成固定第 50 轮标准 bbox 评测和严格重载，BN64 / Scratch 的 AP 为 **29.302 / 26.976**，保留率 **108.62%**，训练 GPU-hours 降低 **67.68%**。周期最佳单列为 BN64 第 40 轮 AP 29.389、Scratch 第 20 轮 AP 31.596。BN64 seed 2 的固定末轮 AP 为 **28.517**，周期最佳为第 35 轮 AP 28.704。COCO Scratch seed 2 已完成，三 seed 汇总见上文。

本次新增的五组记录使用执行提交 ee23d3edf04b3741e1a6441c57a9cc2ff4d2516c、配方 SHA256 c4baac70164f7b91e93d1d164864c35038c3fddfcea2d021a027bf176c3abee0；预算、数据、batch 和评测规则沿用本节合同。各组的种子、数据列表、模型配置、checkpoint、预测与评分 SHA256，以及六 rank 更新和显存，见复现包 results/ 对应实验记录。现已完成 12/12 组固定末轮训练与官方评分，结果表与机器可读文件同步更新。

VisDrone seed 1 两组均完成同一 120 轮合同、548 张官方 MATLAB DET 评测及严格重载。BN64 / Scratch 训练作业分别为 2,170.178 / 4,950.414 秒，固定末轮 AP 保留率为 **70.26%**，训练 GPU-hours 降低 **56.16%**；这是 seed 1 的配对结果，三 seed 汇总见上文。两组执行提交均为 ee23d3edf04b3741e1a6441c57a9cc2ff4d2516c，BN64 / Scratch 末轮 checkpoint SHA256 分别为 894896088a4f85c24c0e4e10675b98ba27b4a76a7053329334ee4dfbbc844ff9 / ba0f6ed0b653f09ff2921c85ce06eaf189f3ed90b3f4d1f440475378c111adf2；复现包 results/visdrone-BN64-seed1.json 与 results/visdrone-SCRATCH-seed1.json 记录完整配置、各 rank 显存和产物校验。训练成本口径沿用下文定义，特征抽取和独立评分单列。



COCO seed 0 两组固定第50轮现已完成标准评测与严格重载。BN64的APs/APm/APl为11.789/30.798/41.191，训练作业6,380.461秒，末轮checkpoint SHA256为 8dd58343ca57fe9453bff232ba76de74f21e0a520c9c6cafb3fbda021a6e0fd6；两组共享本节50轮预算、全量COCO、seed0、640输入、batch384、六卡与BF16/FP32策略。相对Scratch固定末轮，BN64的AP增加0.907个绝对百分点，AP保留率103.33%，训练GPU-hours降低67.68%。周期最佳作为补充：BN64第35轮AP28.308，Scratch第20轮AP31.539；固定预算末轮比较与周期最佳比较分别报告。此处保留 seed 0 明细，后续各 seed 与 VisDrone 三 seed 汇总见上文；训练时间不包含Teacher抽取和独立评测。
...
2026-09-12 首组正式结果：上表 VisDrone 为 seed 0 固定第 120 轮，完整 train 6,471 / val 548，640 输入、batch 96、六卡 A40、BF16 训练/FP32 评测，执行提交 1f940445d4833794aa224fb626d99a8e2ed9703f；配置摘要和种子协议同本节统一合同。官方 DET MATLAB 工具固定 005445782213e20cb91bc50a597db3dd949e749a，548 张覆盖与 checkpoint 严格重载均通过，AP 按 0-100 点报告。冻结组 AP 保留率为 **62.32%**，训练 GPU-hours 降低 **56.61%**；这是 seed 0 的精度/成本折中，VisDrone 三 seed 汇总见上文。上述成本只计训练作业，Teacher 抽取、独立预测及 MATLAB 评分分别列项。Scratch 前40轮与后80轮作业合计5,030.417秒；BN64为2,182.681秒，接管进程的结束时间按10秒轮询观察，保留相应计时误差。两组 MATLAB 单独评分分别为 BN64 29.521秒、Scratch 20.994秒。
...
主表逐 seed 列示，汇总使用三 seed 均值 ± 样本标准差；显存范围、配对区间及完整来源见机器可读记录。

#### VisDrone 周期最佳补充（官方 MATLAB）

在相同 120 轮预算下，对每组第 5/10/.../120 轮共 24 个周期快照进行官方 DET 评分，以 AP 最大者选 standard best，并列时取较早轮次。内部 best 按训练内置指标选择，另列其官方 AP。六组共 150 项补充评分均完成；每项覆盖完整 548 张验证图，模型、预测及报告摘要一致，六组第 120 轮的全部七项 AP/AR 与原固定末轮报告逐项一致。

| 架构 / seed | 周期最佳 epoch | 官方 AP / AP50 / AP75 | AR500 | 内部 best epoch / 官方 AP |
|---|---:|---|---:|---|
| BN64 / 0 | 40 | 7.923 / 18.809 / 5.808 | 21.376 | 38 / 8.111 |
| BN64 / 1 | 50 | 8.230 / 19.050 / 6.311 | 21.658 | 37 / 8.274 |
| BN64 / 2 | 35 | 8.048 / 18.782 / 6.025 | 21.029 | 46 / 7.986 |
| Scratch / 0 | 30 | 14.578 / 28.575 / 13.077 | 29.404 | 23 / 14.849 |
| Scratch / 1 | 25 | 14.377 / 28.366 / 12.740 | 30.482 | 22 / 14.651 |
| Scratch / 2 | 25 | 14.499 / 28.524 / 12.999 | 30.183 | 26 / 14.674 |

周期最佳 AP 的三 seed 均值 ± 样本标准差为 BN64 **8.067 ± 0.154**、Scratch **14.485 ± 0.101**，保留率为 **55.69%**。配对 AP 差为 **-6.418 点**，探索性 95% t 区间 **[-7.053, -5.783]**（n=3、df=2）。两组周期最佳均早于末轮，说明固定末轮与最佳快照应分别解读；GPU 成本仍使用完整 120 轮作业，不能按最佳轮次截短。完整七项指标、150 份报告的来源与 SHA256 见复现包 results/visdrone-periodic-official-summary.json。执行提交、输入、seed、batch 和硬件沿用上文同一合同。
