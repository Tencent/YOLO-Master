...
P1 完成了同预算 KD-off 对照，以及“教师模型 × 蒸馏尺度”矩阵中的 A、B、C 三格。每格和 off 均完成 `17`、`29`、`43` 三个随机种子，共 12 次 400-epoch VOC 训练。D 格按“2×2 中至少完成 3 格”的验收要求跳过。
...
已完成的 A/B 比较蒸馏尺度，A/C 比较教师。D 需要同时改变教师和尺度，在当前预算下提供的信息少于完成第三个独立格，因此未运行。

## 实验可信度与证据

重新归档后的 12 个 `resolved_args.yaml` 通过事后无混杂核查。同一实验格的三个 seed 使用相同的 teacher、蒸馏层级和 `foundation_loss_weight`；不同实验格之间只改变声明的对照轴及运行标识。每个 treatment 均与相同 seed 的 off 配对。
...
|---|---|---:|---|---|
| A - C | 同为 P4 时，DINOv3 是否优于 SigLIP2 | +0.00665 | [-0.01097, +0.02427] | 平均值偏向 DINOv3，但 seed 43 方向相反，结论不确定 |
| A - B | DINOv3 下，P4 是否优于 multiscale | +0.00590 | [-0.02343, +0.03522] | 平均值偏向 P4，但区间很宽，无法确定尺度效应 |

两条轴都没有得到稳定的教师或尺度主效应。A 的均值最高，但其 seed 间方差也最大；仅凭这三次运行不能把差异归因于 DINOv3 P4 的稳定优势。
